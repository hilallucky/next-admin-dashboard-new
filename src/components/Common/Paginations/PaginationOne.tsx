...
const PaginationOne: React.FC<PaginationOneProps> = ({
    ...
    const generatePaginationLinks = () => {
        ...
        const addPageLink = (pageNumber: number) => {
            links.push(
                <li key={pageNumber}>
                    <Link href="#" onClick={() => handlePageChange(pageNumber)} className={`flex items-center justify-center border border-stroke px-3 py-[5px] leading-none ${pageNumber === page ? 'bg-primary text-white' : 'hover:border-primary hover:bg-gray hover:text-primary dark:border-strokedark dark:hover:border-primary dark:hover:bg-graydark'}`}>
                        {pageNumber}
                    </Link>
                </li>
            );
